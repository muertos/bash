...
print_header () {
  echo "timestamp,code,time_total,time_namelookup,time_connect,time_appconnect,time_starttransfer"
}
...
make_request () {
  ...
  echo -n $(date +%s)","
  curl \
    --write-out "%{http_code},%{time_total},%{time_namelookup},%{time_connect},%{time_appconnect},%{time_starttransfer}\n" \
    --silent \
    --output /dev/null \
    "$url"
}
